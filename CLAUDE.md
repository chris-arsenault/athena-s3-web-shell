...
17. **Audit `query.end` dedup is in-memory per proxy process.** `services/audit.ts` holds a `Set<executionId>` so a terminal Athena state emits `query.end` exactly once, no matter how many times the SPA polls. On proxy restart the set empties — a running query that goes terminal while a new task is booting may emit zero `query.end` events. This is acceptable: the authoritative record is in Athena's own query history. If you ever scale to multiple proxy tasks, two tasks seeing the same executionId will each emit one `query.end` — reviewer dedup happens on `(executionId, event)` in the Logs Insights query, not here.
18. **Never log raw SQL.** `services/audit.ts` has `sqlFingerprint()` — strips comments, replaces string literals with `'?'` and numbers with `?`, sha256-hashes the normalized form. The `query.start` audit event carries both the fingerprint (human-readable, safe) and the hash (short, stable for "how many times was this shape run"). If you need the raw SQL for debugging, look it up in Athena's own `get-query-execution` output — Athena holds it for 45 days.
